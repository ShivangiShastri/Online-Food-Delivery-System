...
class IRestaurantServiceImplTestWithMock {
	...
	@Test
	void testAddRestaurantShouldReturnRestaurantObject()
			throws DuplicateRestaurantException, NoSuchRestaurantException {

		/*
		 * Address address = new Address(5, "Srushti", "Kothrud", "10", "Pune", "MH",
		 * "India", "431334"); Category category =new Category(1, "Veg"); Item item1 =
		 * new Item(5, "Manchurian",1,100, category); Item item2 = new Item(6,
		 * "Paneer",1,150, category); Set<Item> items= new HashSet(); items.add(item1);
		 * items.add(item2); Restaurant restaurant = new
		 * Restaurant(5,"Ratna","Aishwarysa","8967541212",address, items);
		 */
	...
	@Test
	void testFindRestaurantByIdShouldThrowNoSuchRestaurantException() {
		assertThrows(NoSuchRestaurantException.class, () -> {
			service.viewAllRestaurants();
		});
	}
